...
@inject("rootStore")
@observer
class InsuredPersonEditComponent extends React.Component<
  Props &
    RootStoreProp &
    WrappedComponentProps &
    MainStoreInjected &
...
> {
  ...
  onChangeInsuranceContract = (insuranceContractId?: string) => {
    ...
    if (
      this.insuranceContractsDc &&
      this.insuranceContractsDc.status !== "LOADING"
    ) {
      this.insuranceContractsDc.items
        .filter(value => value.id === insuranceContractId)
        .forEach(insuranceContract => {
          if (insuranceContract.attachments)
            this.annexes = insuranceContract.attachments
              .map(value => value.attachment)
              .filter(value => value) as FileDescriptor[];
          this.props.form.setFieldsValue({
            insuranceProgram: insuranceContract.insuranceProgram
          });
        });
    }
    ...
    return (
      <Card className="narrow-layout">
          ...
          <Form onSubmit={this.handleSubmit} layout="vertical">
            <Row gutter={16}>
              ...
              <Col span={8}>
                  ...
                  >
                    {this.props.form.getFieldDecorator("statementFile", {
                      initialValue:
                        ...
                        this.dataInstance.item.statementFile
                          ? {
                ...
                <Card
                  size="small"
                  title={<FormattedMessage id="annexes" />}
                  style={card_style}
                >
                  <span style={{ color: "red", paddingLeft: 10 }}>
                    <FormattedMessage id="see.documents" />
                  </span>
                  <br />
                  {this.annexes ? (
                    this.annexes.map(a => (
        ...
        <InsuredPersonMemberComponent
          ...
          insuranceContract={() =>
            this.props.form.getFieldValue("insuranceContract")
          }
  ...
  initPersonMembers = (insuranceContractId?: string) => {
    const contractId =
      insuranceContractId ||
      this.props.form.getFieldValue("insuranceContract") ||
      (this.dataInstance.item && this.dataInstance.item.insuranceContract
        ? this.dataInstance.item!.insuranceContract.id
        : undefined);
    if (contractId)
      restServices.documentService
        ...
        .then(value => {
          ...
          const totalAmount = this.familyDataCollection.items
  ...
  subscribeMemberToMIC = () => {
    this.props.history.push(
      InsuredPersonManagement.PATH + "/" + InsuredPersonManagement.NEW_SUBPATH
    );
  };

  editMicMember = () => {
    this.props.history.push(
      InsuredPersonManagement.PATH + "/" + this.selectedRowKey
    );
  };
  ...
  getRecordById(id: string): SerializedEntity<InsuredPerson> {
    const record:
      ...
      | undefined = this.familyDataCollection.items.find(
      record => record.id === id
    );
    ...
    this.reactionDisposer = reaction(
      ...
      item => {
        ...
        const afterInsuranceContract = () => {
          if (item && item.insuranceContract)
            this.onChangeInsuranceContract(item.insuranceContract.id);
        };

        afterInsuranceContract();
        const personGroupId = item!.employee
          ? item!.employee.id
          : this.props.rootStore!.userInfo!.personGroupId;

        restServices.portalHelperService
          .companiesForLoadDictionary({
            personGroupId: personGroupId as string
          })
          .then(value => {
            restServices.documentService.getMyInsuraces().then(items => {
              const usedInsuranceContracts = items
                ? items
              ...
              this.insuranceContractsDc = collectionWithAfterLoad<
                InsuranceContract
              >(InsuranceContract.NAME, afterInsuranceContract, {
