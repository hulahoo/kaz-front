...
@injectMainStore
@observer
class MyProfile extends React.Component<MyProfileProps & WrappedComponentProps & RootStoreProp & FormComponentProps & MainStoreInjected> {
    ...
    return (
      ...
      <div style={{height: '100%', display: 'flex'}}>
        <div style={{
          ...
          <Card className="narrow-layout large-section section-container" bodyStyle={{textAlign: 'center'}}>
            <img alt={this.person ? this.person.fullName || '' : ''}
                 src={this.urlImg ? this.urlImg : require('../../../resources/img/default-avatar.svg')}
                 style={{height: 200, width: 200, borderRadius: '50%'}}/>
            <h3>{this.person ? this.person.fullName || '' : ''}</h3>
            <p className="title">{this.person ? this.person.positionName || '' : ''}</p>
          ...
          <Card className="narrow-layout large-section section-container"
            ...
            <h1 style={{fontWeight: 'bold'}}><FormattedMessage
              id={'myProfile.contactInformation'}/></h1>
            <div style={hasPhone ? {} : {display: 'none'}}>
              <img alt={''} width={15} src={require("../../../resources/icons/contact/phone-alt-solid.svg")}/>
              <span style={{paddingLeft: 10}}>{this.person ? this.person.phone || '' : ''}</span>
            </div>
            <div style={hasEmail ? {} : {display: 'none'}}>
              <img alt={''} width={15} src={require("../../../resources/icons/contact/envelope-solid.svg")}/>
              {(this.person && this.person.email ?
                <a href={'mailto:' + this.person.email} style={{paddingLeft: 10}}>{this.person.email}</a> : <></>)}
            </div>
          </Card>
            ...
            <List>
              {this.getLeftMenu().map((menu: Menu) =>
                <List.Item
                  ...
                  }}><FormattedMessage id={menu.caption || menu.id}/></List.Item>)}
            </List>
          </Card>

        </div>

        <div style={{float: 'right', display: 'block', height: "100%", width: '100%', overflow: 'auto'}}>
  ...
  renderContent = () => {
    ...
    switch (this.selectedLeftMenu) {
      ...
      case 'personalDataRequest':
        return <PersonalDataRequestList personGroupId={this.props.personGroupId}/>;
