...
@inject("rootStore")
@observer
class EnrollmentListComponent<T> extends React.Component<
  RootStoreProp & WrappedComponentProps
> {
  @observable
  status = "LOADING";
  ...
  onSearch = (value: string) => {
    if (value) {
      restServices.enrollmentService
        .searchEnrollments({
  ...
  render() {
    const { TabPane } = Tabs;
    const { Option } = Select;
    const defaultTabKey = this.props.rootStore!.courseCatalogStore
      ? this.props.rootStore!.courseCatalogStore.selectedEnrollmentId
      : undefined;
            ...
            <div style={{ display: "flex" }}>
              ...
              <div style={{ width: "20%", marginLeft: "30px" }}>
                <Select
                  ...
                  onChange={value => this.handleChange("name", value as string)}
                  style={{ width: "100%" }}
                >
                  <Option value={"1"}>
                    {this.props.intl.formatMessage({ id: "filter.AtoZ" })}
                  </Option>
                  <Option value={"2"}>
                    {this.props.intl.formatMessage({ id: "filter.ZtoA" })}
                  </Option>
                  <Option value={"3"}>
                    {this.props.intl.formatMessage({ id: "filter.Newest" })}
                  </Option>
                  <Option value={"4"}>
                    {this.props.intl.formatMessage({ id: "filter.Oldest" })}
                  </Option>
                </Select>
              </div>
            </div>
            {this.status === "DONE" ? (
              ...
              >
                {this.dataCollection.map(
                  (category: SerializedEntity<EnrollmentCatalogModel>) => (
                    <TabPane tab={category.langValue} key={category.id}>
                          ...
                          {category
                            .courses!.sort((a: any, b: any) => {
                              return this.filterValue === "1"
                                ...
                                : this.filterValue === "3"
                                ? new Date(b.createTs).valueOf() -
                                  new Date(a.createTs).valueOf()
                                : this.filterValue === "4"
                                ? new Date(a.createTs).valueOf() -
                                  new Date(b.createTs).valueOf()
                                : "";
                            ...
                            .map(course => (
                              <Link
                                to={
                                  "/" + CourseManagement.PATH + "/" + course.id
                                }
                              >
                                <PanelCard
                                  key={course.id}
                                    ...
                                    <>
                                      ...
                                      CardIconFactory.getIcon(
                                        ...
                                        ? React.createElement(
                                            ...
                                            {
                                              className: "course-icon left-icon"
                                            }
                                          )
                                        : null}
                                      {course.isOnline ? (
  ...
  tabOnChange = (activeKey: string) => {
    this.props.rootStore!.courseCatalogStore!.setSelectedEnrollmentId(
      activeKey
    );
  };
}
